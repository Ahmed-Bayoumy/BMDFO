...
class airfoil_generator:
  ...
  def create_TSFOIL_sim_files(self, initFile:str = 'Initial.inp', id: int = 0):
    ...
    src_f = os.path.join(os.getcwd(), 'src/BBO/CFD/TSFOIL/'+initFile) 
    dest_f = os.path.join(os.getcwd(), f'src/BBO/CFD/TSFOIL/Design{id}.inp') 
  ...
  def run(self, id: int = 0):
    self.old = os.getcwd()
    os.chdir(os.path.join(self.old, 'src/BBO/CFD/TSFOIL'))
    cmd = './tsfoil'
  ...
  def get_responses(self, id:int = 0, file_name: str = 'Design0', solver: str = "TSFOIL"):
    if solver == "TSFOIL":
      ...
      shutil.move(os.path.join(os.getcwd(), file_name), f'../airfoil/post/Designs/Design{id}.out')
      shutil.move(os.path.join(os.getcwd(), f'Design{id}.inp'), f'../airfoil/post/Designs/Design{id}.inp')
      os.chdir(self.old)
...
if __name__ == "__main__":
  ...
  param = {"baseline":     [0.06, 0.5, 0.2, -0.07, 0.5, -0.05, -0.004],
              ...
              "scaling": 1,
              "post_dir": "/Users/ahmedb/apps/code/Bay_dev/NOBM/src/BBO/CFD/airfoil/post"} # You need to define an absolute path to be able to get the MADS.csv results file
  options = {
        "seed": 0,
        "budget": 1000,
        "tol": 1E-4,
